...
def log_validation_failure(question: str, sql: str, error_msg: str) -> None:
    ...
    # Extract candidate tables (schema.table pattern)
...
def validate_query(sql: str, engine: Engine) -> ValidationResult:
    ...
    if not sql or not sql.strip():
        return ValidationResult(valid=False, error="Empty SQL query provided.")

    if not is_read_only(sql):
        return ValidationResult(
            valid=False,
            error="Security Violation: Query contains prohibited DML/DDL statements or non-SELECT keywords."
        )
    ...
    try:
        with engine.connect() as conn:
            # Run EXPLAIN to validate syntax, schemas, tables, and columns against live catalog
            result = conn.execute(text(f"EXPLAIN {clean_sql}"))
            plan_lines = [str(row[0]) for row in result.fetchall()]
            return ValidationResult(valid=True, explain_plan="\n".join(plan_lines))
    except Exception as e:
        error_msg = str(e)
...
def execute_and_validate_with_retry(
    ...
    Caps retries at max_retries (default 2, meaning up to 3 total attempts).
    """
    last_error = None
    attempted_sqls = []

    for attempt in range(max_retries + 1):
        # Generate SQL (pass previous error if retry)
        ...
        log_validation_failure(question, sql, last_error)
        print(f"[auditor] Attempt {attempt + 1} validation failed: {last_error[:120]}...")
